...
const char *interfacePortName = "COM8";
HANDLE m_interfacePortHandle = NULL;
...
BOOL TestApp::openTestSerialPort() {
		...
		if (!flgMainPortOpen){							// Get current configuration of serial communication port.
			...
			int tryAgain;
			do {
				m_interfacePortHandle = CreateFile(interfacePortName,  // Specify port device: default "COM1"		
				...
				if (GetCommState(m_interfacePortHandle, &m_testPortConfig) == 0) {
			...
			if (SetCommState(m_interfacePortHandle, &m_testPortConfig) == 0)
			{
				flgMainPortOpen = FALSE;
				// intError = PORT_ERROR;
			...
			SetCommTimeouts(m_interfacePortHandle, &comTimeOut);		// set the time-out parameter into device control.
	...
	BOOL TestApp::closeTestSerialPort() {
		if (flgMainPortOpen)               // Port need to be open before.
		{
			...
			if (!CloseHandle(m_interfacePortHandle))   // Call this function to close port.
			{
	...
	BOOL TestApp::WriteSerialPort (int targetDevice, char *ptrPacket){
		...
		do {
			trial++;
			if (WriteFile(m_interfacePortHandle, ptrPacket, length, (LPDWORD) &numBytesWritten, NULL)) break;
			msDelay(100);
	...
	BOOL TestApp::ReadSerialPort(int targetDevice, char *ptrPacket) {
		...
		do {
			trial++;
			msDelay(100);
			if (ReadFile(m_interfacePortHandle, inBytes, BUFFERSIZE, &numBytesRead, NULL)) {
